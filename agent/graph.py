# agent/graph.py (solo las partes relevantes)
...
from agent.nodes.capture_user_taxon import capture_user_taxon  # <- NUEVO
...
def build_graph():
    ...
    g.add_node("capture_user_taxon", capture_user_taxon)  # <- NUEVO

    # Entry
    g.add_edge(START, "router_input")

    # Router -> classify o pedir imagen (imagen obligatoria)
    ...
    g.add_conditional_edges(
        ...
        route_selector,
        {
            ...
            "ask_image": "capture_user_taxon",  # <- primero capturamos taxón del usuario
        },
    )
    ...
    g.add_conditional_edges("ensure_image", img_ok, {
    ...
    g.add_edge("qa_about_taxon", END)  # (se usaría sólo tras finalizar si decides añadir un paso Q&A)
